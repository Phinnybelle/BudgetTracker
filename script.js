...
addButton.addEventListener("click", function(){
    ...
    if (item && !isNaN(amount) && date){
        ...
        row.innerHTML = `
        <td class="border border-black p-2">${item}</td> 
        <td class="border border-black p-2">${displayAmount}</td>
        <td class="border border-black p-2">${category}</td>
        <td class="border border-black p-2">${date}</td>
        <td class="border border-black p-2"> <button class="deleteBtn ml-10 bg-red-500 text-white px-2 py-1 rounded" >Delete</button></td>
           `;
